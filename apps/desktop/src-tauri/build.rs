...
fn main() {
    // In release builds, copy the pre-built hitch-daemon into src-tauri/binaries/
    // with the target-triple suffix that Tauri's externalBin expects.
    // The daemon must be built first: `cargo build -p hitch-daemon --release`.
    if std::env::var("PROFILE").unwrap_or_default() == "release" {
        ...
        let daemon_src = workspace_root.join("target/release/hitch-daemon");
        if daemon_src.exists() {
            ...
            println!(
                "cargo:warning=hitch-daemon not found at {}; run `cargo build -p hitch-daemon --release` first",
                daemon_src.display()
            );
        }
    }
