...
Request:
  ...
  git-log(worktree_id, limit, offset)
  commit-diff(worktree_id, commit_id)
  ...
  list-draft-models(provider, settings?)
  generate-commit-draft(worktree_id, settings? {provider, model?, claude_path?, codex_path?})
  generate-pull-request-draft(worktree_id, base?, settings? {provider, model?, claude_path?, codex_path?})
  push(worktree_id)
  fetch(worktree_id)
  pull(worktree_id)
...
Response:
  ...
  pr-status(pr?)
  file-diff(diff)
  commit-log(commits, has_more)
  commit-diff(meta, files)
